...
function getSettlementRedTooltipSpec(state) {
  ...
  const redCap = Math.max(0, population.total - population.staffed);
  return {
    title: "Red Resource",
    lines: [
      `Current stockpile: ${Math.floor(red)}/${redCap}`,
      "Generated by Flood Rites and capped by the population not tied to structures.",
    ],
  };
}
...
function getSettlementFaithTooltipSpec(state, classId = null) {
  ...
  return {
    ...
    lines: [
      ...
      `Growth streak: ${faith.growthStreak}/${faith.growthThreshold}`,
      "Prosperous years grow faith; starvation years degrade it.",
    ],
  };
}
