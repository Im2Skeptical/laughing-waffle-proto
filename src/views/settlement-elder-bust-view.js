...
function portraitSeedSource(portrait) {
  return [
    portrait?.skinTone ?? "",
    portrait?.hairStyle ?? "",
    portrait?.hairColor ?? "",
    portrait?.faceShape ?? "",
    portrait?.clothingColor ?? "",
    portrait?.accessory ?? "",
    portrait?.expression ?? "",
  ].join("|");
}

export function drawDeterministicBust(container, rect, portrait, {
  outlineColor = PALETTE.stroke,
} = {}) {
  const root = new PIXI.Container();
  root.position.set(rect.x, rect.y);
  const seed = hashString(portraitSeedSource(portrait));
  const skinTone = ELDER_BUST_SKIN_TONES[seed % ELDER_BUST_SKIN_TONES.length];
  const accentTone = ELDER_BUST_ACCENT_TONES[
    Math.floor(seed / 7) % ELDER_BUST_ACCENT_TONES.length
  ];
  const darkTone = PALETTE.bustDark;

  const backdrop = new PIXI.Graphics();
  roundedRect(backdrop, 0, 0, rect.width, rect.height,
    Math.min(18, Math.floor(rect.height * 0.35)),
    PALETTE.bustBackdrop, outlineColor, 2);
  ...
  if (shoulderVariant === 0) {
    shoulders.drawRoundedRect(rect.width * 0.12, rect.height * 0.52,
      rect.width * 0.76, rect.height * 0.32, 10);
    ...
    shoulders.drawEllipse(rect.width * 0.5, rect.height * 0.73,
  ...
  if (headVariant === 0) {
    head.drawEllipse(rect.width * 0.5, rect.height * 0.33,
      rect.width * 0.17, rect.height * 0.2);
  } else if (headVariant === 1) {
    head.drawRoundedRect(rect.width * 0.33, rect.height * 0.13,
      rect.width * 0.34, rect.height * 0.42, 12);
  } else {
    head.drawCircle(rect.width * 0.5, rect.height * 0.32,
  ...
  if (hairVariant === 0) {
    hair.drawEllipse(rect.width * 0.5, rect.height * 0.23,
      rect.width * 0.2, rect.height * 0.13);
  } else if (hairVariant === 1) {
    hair.drawRoundedRect(rect.width * 0.3, rect.height * 0.09,
      rect.width * 0.4, rect.height * 0.16, 10);
    ...
    hair.drawEllipse(rect.width * 0.5, rect.height * 0.2,
      rect.width * 0.22, rect.height * 0.1);
    hair.drawRect(rect.width * 0.46, rect.height * 0.13,
  ...
  if ((Math.floor(seed / 89) % 2) === 0) {
    ...
    diadem.lineStyle(Math.max(2, rect.width * 0.02), PALETTE.accent, 0.9);
