...
The implemented debug-tool expansion and its replay/schema impact analysis are
in [`ai/debug-tools-expansion-plan.md`](./debug-tools-expansion-plan.md).
...
- Game state is schema v6 (`gameStateSchemaVersion: 6`).
- Runner saves use schema v6.
- Each run stores schema-v1 Game Settings and Gamepieces documents in its
  JSON-only `gameConfig`.
...
Development Tools also includes:

- **Game Settings**, generated from the active setting registry. It covers
  timing, worker policy, meal consumption and decay, demographic probability
  tables, elder mortality, happiness/starvation/housing, chaos/loss, Elder
  Order resistance, and vassal generation.
- **Gamepieces**, generated by traversing all detailed structure and practice
  definitions. It exposes numeric structure scaling, worker/charge values, and
  DSL effect parameters, so newly authored detailed pieces appear
  automatically.
- **Vassal Lab**, which injects a fully specified target, lifespan, trait,
  profession, resistance snapshot, and ordered interventions at the viewed
  second. The action is replayable and consumes no RNG; replacing a living
  vassal must be explicitly enabled.

Game Settings and Gamepieces have independent schema-v1 browser drafts and
named preset libraries, plus JSON import/export. Browser drafts are inert.
Starting a fresh test run combines them with the current Map Lab draft and
copies the canonical result into `GameState`. Simulation and definition lookup
read only that state-scoped configuration, never browser storage or mutable
module globals.
...
Tests cover configuration validation, generated definition fields,
state-scoped tuning, deterministic vassal injection, definitions, worker
assignment, map scores, boundary food behavior,
